/**
 * CHAMADO GLPI 
 * SM2CE - ALTERADO EM 06.03.24 PQ ESTAVA INICIANDO COMO 'tabela' E GERANDO ERRO PARA O TIPO DE CHAMADO
...
var configApp = {
    ...
    mounted: function () { //Executa quando a aplicação é montada
        ...
        if (this.atividadeAtual == this.process.inicio || this.atividadeAtual == 0) {
            ...
            // SM2CE - RICARDO ANDRADE
            // DESLIGADO EM 29.05.24 - TEAMS COM PAULO E MORAES 
    ...
    methods: {
        ...
        addFornecedores: function (listaFornecedor) {
            ...
            if (listaFornecedor && listaFornecedor.length > 0) {
                try {
                    ...
                    fornecedores = getInDataset("ds_Fornecedores", null, buscaFornecedores);
        ...
        addCotacao: function (listProduto, fornecedor) {
            ...
            for (var i in listProduto) {
                ...
                // Adicionado para setar o valor quando apenas salvar - Lucca Balona 26/03/24
        ...
        configCotacao: function (row) {
            ...
            if (self.form.tipoCotacao == 'fechada') {
                addMaskMonetariaZero("#valorCotacao___" + row,4);
            } else {
                addMaskMonetaria("#valorCotacao___" + row,4);
            }
        ...
        setValorCotacao: function (codFornecedor, codProduto, valor) {
            readColumn("codCotacao", function (row, item) {
                if (codProduto == item.val() && codFornecedor == $("#codFornecedorCotacao___" + row).val()) {
                    $("#valorCotacao___" + row).val(addMascaraMonetaria(valor,4)).change();
                    return false;
                }
