    ...
    if (numState == 6) { 
    	...
    	if (qnt > 1) { // validar campos obrigatórios
			$("input[id^='alt_desc___']").each(function(index) {
				...
				throw "Por favor preencher a justificativa de alteração na linha: "+id
...
function verificaFornecedorSemCadastro() {
    ...
    if (subprocessos > 0) {
        ...
        if (contadorProcessoAtivo > 0) {
            ...
            throw ("Não é possível prosseguir com o chamado pois ainda existe(m) " + contadorProcessoAtivo +
        ...
        (dadosParaCadastroFornecedor.length <= 0) ? $('#iniciarCadastro').val('false'): $('#iniciarCadastro').val('true');
