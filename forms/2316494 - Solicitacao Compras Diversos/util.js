    ...
    callbackFactory.success = function (data) {
        ...
        myTableFC = FLUIGC.datatable('#tabCONSULTA', {
            ...
            if (data)
                dataAll = data;
            if (err) {
                avisoMODAL('Procurando', 'Falha ao carregar consulta. \n ' + JSON.stringify(err));
            } else
                $('#datatable-area').removeAttr('class');
        });
    }
...
// VALIDAR PRODUTO SELECIONADO CONF REGRAS FILIAS
function seProdutoOk(qCODIGO,item,index) {
    var produtoExistente = false;
    $("[name^=codProd___]").each(function() {
       if (qCODIGO == $(this).val() && parseInt(qCODIGO) <= 99000) {
          // produtoExistente = true;
       }
    });

    if (produtoExistente) {
       showMessage("Atenção!", "Já foi encontrado o produto <b>" + qCODIGO + " - " + item.DESCRICAO + "</b> na lista de produtos.");
       ...
       if (parseInt(qCODIGO) >= 990000 /*&& $("#tipoCompra").val() != "3"*/ ) {
          FLUIGC.message.alert({
       ...
       } else if (parseInt(qCODIGO) > 0 && parseInt(qCODIGO) <= 880000 && ($("#codFilial").val() == '07401' || $("#codFilial").val() == '08401' || $("#codFilial").val() == '12201' || $("#codFilial").val() == '06601' || $("#codFilial").val() == '02901' || $("#codFilial").val() == '07301') && $("#tipoCompra").val() == "2") {
          FLUIGC.message.alert({
       ...
       } else if (parseInt(qCODIGO) > 880000 && ($("#codFilial").val() != '07401' && $("#codFilial").val() != '08401' && $("#codFilial").val() != '12201' && $("#codFilial").val() != '06601' && $("#codFilial").val() != '02901' && $("#codFilial").val() != '07301') && $("#tipoCompra").val() == "2") {
          FLUIGC.message.alert({
          ...
          if (tipo == "SV" && pegaCod > "000100") {
             window["codProd___" + index].values = qCODIGO;
...
$(document).on("click", "[confirma_CLASSIFICACAO]", function (evCLASSIFICACAO) {
    ...
    if (qualCampoId.replace('___'+x1, '') == 'nomeProd') {
        ...
        // VALIDAR PRODUTO SELECIONADO CONF REGRAS FILIAS
        if(seProdutoOk(selected['CODIGO'],selected,x1) == false) 
            return false;
        ...
        var ultimo_preco = parseFloat(selected['ULTIMO_PRECO'].replace(".",","));
	        if(isNaN(ultimo_preco))
	        	ultimo_preco = 0;
	        if(ultimo_preco <= 0)
	        	$('#valorProd'      +x1).prop('readonly', false).prop('disabled', false);
        ...
        */
    	...
    	$("#descCondPgtoItemCotacao___"+x1).val(selected['DESCRICAO']);
