...
var configApp = {
  ...
  data: {
    ...
    form: {
      ...
      //ValidarCursos
      motivoAprovValidarCursos: $("#motivoAprovValidarCursos").val(),
      decisaoAprovValidarCursos: $("#decisaoAprovValidarCursos").val(),
      ...
      idAprovValidarCursos: $("#idAprovValidarCursos").val(),
      nomeAprovValidarCursos: $("#nomeAprovValidarCursos").val(),
      dataAprovValidarCursos: $("#dataAprovValidarCursos").val(),
      motivoAprovValidarCursos: $("#motivoAprovValidarCursos").val(),
  ...
  mounted: function () {
    //Executa quando a aplicação é montada
    var self = this;
    ...
    if (
      this.atividadeAtual != this.process.inicio ||
      this.atividadeAtual != this.process.analiseCompradorCotacao
    ) {
      document.getElementById("addProd").disabled = false;
    }

    if (this.atividadeAtual == this.process.ValidarCursos) {
    ...
    if (
      ...
      $(".fixedTopBar, #page-header", parent.document)
        .find("button:first")
        .bind("click", function () {
          var codsIss = "";
      ...
      $('.contratoSimTI').hide();
      $('#uniContratoAtivoTI').click(function () {
        if (this.value == "naoContratoTI") {
          $('.contratoSimTI').hide();
        }
        if (this.value == "simContratoTI") {
          $('.contratoSimTI').show();
        }
      ...
      addEventSendFluig(function () {
        ...
        if (
          self.form.isFornecedorExclu == true &&
          self.form.isProdutoTI == "nao"
        ) {
          self.setAlcadaAprovacaoCarta();
        } else if (
          self.form.isFornecedorExclu == true &&
          self.form.tipoCompra == "5"
        ) {
          self.setAlcadaAprovacaoCarta();
        }
      });
      ...
      if (
        $("#avancaGestor").val() == "false" &&
        $("#justificaCotacaoMaiorCartaEx").val() == ""
      ) {
      ...
      if ($("#tipoCompra").val() == "5") {
        $("#tomarCienciaObra").val(
          solicitanteDifGrupoObras(parent.WCMAPI.userCode)
        );
      }
      ...
      $("#target").blur(function () {
        ...
        valor > 100000 ?
          $("#executorAtv282").val("Pool:Group:HOFGES") :
          $("#executorAtv282").val("Pool:Group:HOFAC");
      });
      ...
      if (MODO_EDICAO != "VIEW") {
        ...
        if (self.form.tipoCotacao == "tabela") {
          ...
          if (self.listProdutoFechada.length > 0) {
            var dsComprasDivers = getInDataset(
              "ds_comprasDiversos",
              ["codSolicitacao"],
              ["codSolicitacaoVinculada|" + self.form.codSolicitacao]
            );
            if (dsComprasDivers.values.length > 0) {
              self.form.codSolicitacaoVinculada =
                dsComprasDivers.values[0]["codSolicitacao"];
            } else {
              //INICIA SOLICITACAO VINCULADA DA COTACAO FECHADA
              var xmlNovaSolicitacao = renderXmlSolicitacaoDiversos(
                self,
                self.listProdutoFechada,
                "cotacaoFechada"
              );
              self.iniciaNovaSolicitacao(
                xmlNovaSolicitacao,
                //SUCCESS
                function (data) {
                  $(data)
                  ...
                  if (self.form.codSolicitacaoVinculada != "") {
                    FLUIGC.toast({
                      message: "A solicitação " +
                        self.form.codSolicitacaoVinculada +
                        " foi gerada a partir dessa para cotação fechada.",
                      type: "success",
                      timeout: "slow",
                    });
                  } else {
                    showMessage(
                      "ERRO",
                      "Erro para classificar a solicitação entre fechada e tabela de preço. Atualize a página ou contate o time de TI"
                    );
                    console.log(data);
                  }
                },
      ...
      if (
        ...
        $("#decisaoAlcada" + contadorAlcada).val() == "nao"
      ) {
        $(
            ".navTabsAlcada li:nth-child(" +
            contadorAlcada +
            "), .navTabsAlcada div:nth-child(" +
            contadorAlcada +
            ")"
          )
          .addClass("active")
      ...
      if (
        ...
        $("#decisaoAlcadaCarta" + contadorAlcada).val() == "nao"
      ) {
        $(
            ".navTabsAlcadaCarta li:nth-child(" +
            contadorAlcada +
            "), .navTabsAlcadaCarta div:nth-child(" +
            contadorAlcada +
            ")"
          )
          .addClass("active")
      ...
      if (
        ...
        $("#decisaoAlcadaPA" + contadorAlcada).val() == "nao"
      ) {
        $(
            ".navTabsAlcadaPA li:nth-child(" +
            contadorAlcada +
            "), .navTabsAlcadaPA div:nth-child(" +
            contadorAlcada +
            ")"
          )
          .addClass("active")
      ...
      btnAprov(true);
    } else if (this.atividadeAtual == this.process.ValidarCursos) {
      var self = this;
      //self.setAlcadaAprovacaoCarta();
      self.form.motivoAprovValidarCursos = "";
      self.form.decisaoAprovValidarCursos = "";
      //CONFIGURACOES DOS BOTOES DA ALCADA DE APROVACAO
      flagAllBtnAprov();
      self.form.idAprovValidarCursos = parent.WCMAPI.userCode;
      self.form.nomeAprovValidarCursos = parent.WCMAPI.user;
      self.form.dataAprovValidarCursos = getCurrentDate();
      btnAprov(true);
      ...
      flagAllBtnAprov();
      btnAprov(true);
      self.form["dataAprovAlcadaCarta" + contadorAlcadaCarta] =
        getCurrentDate();
      self.form["nomeAprovAlcadaCarta" + contadorAlcadaCarta] =
        parent.WCMAPI.user;
      $(
          ".navTabsAlcadaCarta li:nth-child(" +
          contadorAlcadaCarta +
      ...
      if (
        contadorAlcadaCarta < 5 &&
        self.form["idAprovAlcadaCarta" + contadorAlcadaCarta] ==
      ...
      if (
        contadorAlcada < 8 &&
        self.form["idAprovAlcada" + contadorAlcada] ==
      ...
      // ACAO TEMPORARIA PARA ABRIR TELA DE APROVACAO
      // 02.08.2024 SM2CE - EMERGENCIAL
      $("#fieldsetAlcadaAprovacao").show();
      ...
      if (
        contadorAlcada < 4 &&
        self.form["idAprovAlcadaPA" + contadorAlcada] ==
      ...
      addEventSendFluig(function () {
        ...
        $("#tbPedidosGerados tbody tr:gt(0)").each(function () {
          var emailFornVencedorProd = $.trim(
            $(this).find("[name^=emailFornVencedorProd___]").val()
          );
      ...
      //CONFIGURACOES DOS BOTOES DA ALCADA DE APROVACAO
      altBtnAprov(
        $("#decisaoSolicitante").val(),
        "decisaoSolicitanteBtnAprov",
        "decisaoSolicitanteBtnReprov"
      );
    } else if (
      this.atividadeAtual == this.process.tratativaErroStartAcompanhemento
    ) {
      //BOTÃO DE ENVIAR
      addEventSendFluig(function () {
        dadosVencedores();
      ...
      this.atividadeAtual == this.process.analisarVinculacaoProdFornec
    ) {
      $("#fieldsetObsFiscal").show();
    } else if (
      this.atividadeAtual == this.process.correcaoSolicitante ||
      this.atividadeAtual == 5
    ) {
      //CONFIGURACOES DOS BOTOES DA ALCADA DE APROVACAO
      flagAllBtnAprov();
      definePrioridade();
      ...
      addEventSendFluig(function () {
        ...
        if (
          self.form.alteraValorCartaExecao == "true" ||
          self.form.isFornecedorExclu == true
        ) {
          /// Valida se passara novamente pela carta de exeção
          self.setAlcadaAprovacaoCarta();
        }
        self.setAlcadaAprovacaoDiversos();
      });
    ...
    if (
      ...
      if ($("#isContratoGuardChuva").is(" :checked") == true) {
        var removeLixeiraTbFiliaisGuard =
          $("table")[0].tHead.firstElementChild.children[0];
    ...
    if (
      ...
      var removeLixeiraTbProd =
        $("table")[1].tHead.firstElementChild.children[0];
    ...
    if (
      ...
      var removeLixeiraTbProd =
        $("table")[4].tHead.firstElementChild.children[0];
      removeLixeiraTbProd.hidden = true;
    ...
    FLUIGC.switcher.init("#isPedidoGuardChuva");
    ...
    self.form.isContratoGuardChuva = FLUIGC.switcher.getState(
      "#isContratoGuardChuva"
    );
    FLUIGC.switcher.onChange("#isContratoGuardChuva", function (event, state) {
      ...
      if (state == true) {
        showMessage(
          "Atenção",
          "Minuta contratual contempla mais de um <b>CNPJ CONTRATANTE</b> para o mesmo <b>CNPJ CONTRATADO</b>. Caso haja dúvidas, gentileza entrar em contato com a Central de Atendimento, <b>(31) 3308-8080 </b> ou <b> atendimento.cso@oncoclinicas.com</b>."
        );
    ...
    self.form.isPedidoGuardChuva = FLUIGC.switcher.getState(
      "#isPedidoGuardChuva"
    );
    FLUIGC.switcher.onChange("#isPedidoGuardChuva", function (event, state) {
      self.form.isPedidoGuardChuva = state;
      $("#hidePedidoGuardChuva").val(state);
      if (state == true) {
        $("#divfiliasPedido").show();
          // desativado em 04.07.24 porque nao identifiquei a inicializacao da var row
//        $("#filialGuardChuPedido___" + row)
//          .parent()
//          .parent()
//          .children()
//          .find("i")
//          .hide();
//        window["filialGuardChuPedido___" + row].disable(true);      
      } else {
        $("table[tablename=tbFiliaisPedido] tbody tr").not(":first").remove();
        $("#totalPercFilialPedido").val("0");
        $("#divfiliasPedido").hide();
      }
    });

    self.form.isFornecedorExclu =
      FLUIGC.switcher.getState("#isFornecedorExclu");
    ...
    self.form.isFornecedorExcluClassica = FLUIGC.switcher.getState(
      "#isFornecedorExcluClassica"
    );
    FLUIGC.switcher.onChange(
      "#isFornecedorExcluClassica",
    ...
    self.form.chkMinutaContrato =
      FLUIGC.switcher.getState("#chkMinutaContrato");
    ...
    FLUIGC.switcher.onChange("#isContrato", function (event, state) {
      ...
      if (state == true) {
        $("#nomeSolicitAbertaFilho").val(
          "Numéro da Solicitação Aberta no Fluxo Novo Contrato ou Aditivos:"
        );
        $(".numRgContrato").show();
      } else {
        $("#nomeSolicitAbertaFilho").val("");
    ...
    self.form.isNovoContratoOuAdt = FLUIGC.switcher.getState(
      "#isNovoContratoOuAdt"
    );
    ...
    self.form.isReajusteContrato = FLUIGC.switcher.getState(
      "#isReajusteContrato"
    );
    ...
    $(".compraServico").toggle(
      $("#tipoCompra").val() == "1" || $("#tipoCompra").val() == "4"
    );
    $(".compraDiversos").toggle($("#tipoCompra").val() == "2");
    $(".compraTi").toggle($("#tipoCompra").val() == "3");

    if ($("#tipoCompra").val() == "2" || $("#tipoCompra").val() == "3") {
      $(".pedidoGuardChuva").show();
      $(".contratoGuardChuva").hide();
      $(".isContrato").hide();

    }
    /*
    $(".contratoGuardChuva").toggle($("#tipoCompra").val() == "2");
    $(".pedidoGuardChuva").toggle(!($("#tipoCompra").val() == "2"));
    */
  ...
  methods: {
    ...
    addFilialPedido: function () {

      let quantidadeProdutos = $("#tbProd tbody tr").length
      if (quantidadeProdutos < 2) {
        FLUIGC.toast({
          message: "Antes de adicionar as filiais para pedido de múltiplas partes, você deve adicionar pelo menos 1 produto. " +
            self.form.codSolicitacaoDesvinculada,
          type: "warning",
          timeout: "slow",
        });
      } else {
        var tabela = "tbFiliaisPedido";
        var index = wdkAddChild(tabela);
      }
    },

    gerarTbFiliaisQtdPed: function () {

      $('#tbFiliaisQtdPedido tbody tr').not(':first').remove();

      $('input[id^="cnpjFilialPedido___"]').each(function (index, value) {
        var seq = $(this).attr("id").split("___")[1];

        var filialguardachuva = $('#filialGuardChuPedido___' + seq).val();
        var codguardachuva = $('#codFilialGuardChuPedido___' + seq).val();

        $('input[id^="nomeProd___"]').each(function (index, value) {
          var seq2 = $(this).attr("id").split("___")[1];

          var nomeproduto = $('#nomeProd___' + seq2).val();
          var codproduto = $('#codProd___' + seq2).val();




          var row = wdkAddChild('tbFiliaisQtdPedido');
          $("#filialQtdPedido___" + row).val(filialguardachuva);
          $("#codFilialQtdPedido___" + row).val(codguardachuva);
          $("#produtoQtdPedido___" + row).val(nomeproduto);
          $("#codProdutoQtdPedido___" + row).val(codproduto);
      ...
      if (produto.type != "click") {
        ...
        readColumn("codProd", function (rowProduto, item) {
          if (item.val() == produto.codProd) {
            ...
            var somaQtd =
              parseInt(produto.qtdProd) +
              parseInt($("#qtdProd___" + rowProduto).val());
            $("#qtdProd___" + rowProduto)
              .val(somaQtd)
              .change();
            $("#itemProd___" + rowProduto).val(
              rowProduto + " - " + produto.codSolicitacao
            );
            return false;
          }
        });
        if (!produtoExistente) {
          ...
          $("#fornecedorVencedorProd___" + row).val(
            produto.fornecedorVencedorProd
          );
          $("#itemProd___" + row).val(row + " - " + produto.codSolicitacao);
          $("#itemProd___" + row)
            .parent()
            .removeClass("min-table-field");
          $("#itemProd___" + row)
            .parent()
            .addClass("med-table-field");
          ...
          self.somaTotalProduto(row);
          if (self.form.tipoCotacao == "tabela") {
            self.carregaValorProduto([
              "PRODUTO|" + $("#codProd___" + row).val(),
            ]);
          }
        }
      } else {
        ...
        /* comentado andre 09/08
        if (filial == '07401' || filial == '08401' || filial == '12201'){
          //var c1 = DatasetFactory.createConstraint("filial", '07401', '07401', ConstraintType.MUST);
        //var dataset = DatasetFactory.getDataset("ds_produto", null, new Array(c1), null);      	
        reloadZoomFilterValues("codProd___" + row,'filial,07401');
        }
        */
        $("#itemProd___" + row).val(row);

        if (CURRENT_STATE == 6) {
          // na atividade do comprador, caso adicione um produto, identificara q ele é novo
          $("#newProd___" + row).val("sim");

          readColumn("codProd", function (row) {
            // funcao para somar os novos produtos inseridos na atividade da cotação
            addMaskMonetaria("#valIpiProd___" + row);
            $("#qtdProd___" + row)
              .add("#valorProd___" + row)
              .change(function () {
    ...
    addAglutinacao: function () {
      ...
      $("#codAgl___" + row).change(function () {
        ...
        if (codSolicitacao == self.form.codSolicitacao)
          algutinacaoExistente = true;
        //ALGUTINAÇÃO REPETIDA
        if (algutinacaoExistente) {
          showMessage(
            "Atenção",
            "Essa solicitação já foi aglutinada.",
            function () {
          ...
          try {
            var idTable = getInDataset(
              "ds_comprasDiversos",
              ["documentid"],
              ["codSolicitacao|" + codSolicitacao]
            ).values["0"].documentid;
            var listProduto = getInDataset("ds_comprasDiversos", null, [
              "documentid|" + idTable,
              "tablename|tbProd",
            ]).values;
        ...
        if (codFornecedor.type != "click") {
          try {
            ...
            if (!fornecedorExistente) {
              var fornecedor = getInDataset("ds_Fornecedor", null, [
            ...
            showMessage(
              "Erro",
              "Ocorreu um erro ao tentar buscar o fornecedor. " + e
            );
          }
        } else {
          row = wdkAddChild("tbFornecedor");
    ...
    addDesvinculado: function (desvinculado) {
      ...
      $("#codFornVencedorProdDesvinculado___" + row).val(
        desvinculado.codFornVencedorProd
      );
      if (desvinculado.isFechada == "on")
        $("#isFechadaDesvinculado___" + row).prop("checked", true);
    ...
    addCotacao: function (listProduto, fornecedor) {
      ...
      for (var i in listProduto) {
        ...
        console.log("add cotacao");
        console.log("produto.item " + produto.item);





        $("#codCotacaoLinha___" + row).val(produto.item);
      ...
      $("#hiddenQtdCotacoes").val(
        parseInt($("#tbFornecedor").find("tbody tr").length) - 1
      );
      removeFornecedor();
    },
    openCotacao: function (item, tipoTabela) {
      if (MODO_EDICAO == "VIEW") {
        if (tipoTabela == "produto") {
          var row = $(item)
            .closest("tr")
            .find('[name^="nomeProd___"]')
            .prop("id")
            .split("___")[1];
          this.showModalCotacao(
            "codCotacao",
            $("span#codProd___" + row).text(),
            "COTAÇÃO POR PRODUTO",
            $("span#nomeProd___" + row).text(),
            $("span#itemProd___" + row).text()
          );
        } else if (tipoTabela == "fornecedor") {
          var row = $(item)
            .closest("tr")
            .find('[name^="codForn___"]')
            .prop("id")
            .split("___")[1];
          if ($("span#cnpjForn___" + row).text() != "") {
            this.showModalCotacao(
              "cnpjFornCotacao",
              $("span#cnpjForn___" + row).text(),
              "COTAÇÃO POR FORNECEDOR",
              $("span#nomeForn___" + row).text()
            );
          }
        }
      } else {
    ...
    closeCotacao: function () {
      ...
      if (
        rowJustificativa != null &&
        $("#justificativaCotacao___" + rowJustificativa).val() == ""
      ) {
        showMessage(
          "Atenção",
          "Justificativa é obrigatória para cotações alteradas."
        );
      } else {
    ...
    configCotacao: function (row) {
      ...
      console.log("CONFIG COTACAO ROWW " + row);
      ...
      $("#valorCotacao___" + row).change(function () {
        ...
        var codProdutoAtualLinha = $("#codCotacaoLinha___" + rowChangeValor).val();
        var menorValor = null;
        ...
        readColumn("codCotacao", function (rowCotacao, item) {
          console.log("entrou busca menor valor");

          if (codProdutoAtual == item.val() && codProdutoAtualLinha == $("#codCotacaoLinha___" + rowCotacao).val()) {
            console.log("entrou busca menor valor if");
            valorAtual = removeMascaraMonetaria($("#valorCotacao___" + rowCotacao).val());



            if (valorAtual != 0 && (valorAtual < menorValor || menorValor == null)) {
              console.log("entrou busca menor valor if 2");
        ...
        if (menorValor != null) {
          self.checkVencedor(linhaVencedora, codProdutoAtualLinha);
          readColumn("codProd", function (rowProduto, item) {


            console.log("menorValor diferente null");


            if (item.val() == codProdutoAtual && codProdutoAtualLinha == rowProduto) {
              console.log("entrou seta valor cotacao vencedora if");
      ...
      $("#vencedorCotacao___" + row).change(function () {
        ...
        readColumn("valorCotacao", function (rowAtual, item) {
          if (rowChange != rowAtual && $("#codCotacao___" + rowAtual).val() == $("#codCotacao___" + rowChange).val()) {
            ...
            if (valorVencedorAlterado > cotacaoAtual
              /*removeMascaraMonetaria(item.val()) && removeMascaraMonetaria(item.val()) != 0.00 || $("#justificativaCotacao___" + rowChange).val() != ''*/
            ) {
        ...
        var bool = false;
        readColumn("codProd", function (rowProduto, item) {
          if (item.val() == $("#codCotacao___" + rowChange).val() && codProdutoAtualLinha == rowProduto) {
    ...
    setValorCotacao: function (codFornecedor, codProduto, valor) {
      readColumn("codCotacao", function (row, item) {
        if (
          codProduto == item.val() &&
          codFornecedor == $("#codFornecedorCotacao___" + row).val()
        ) {
          $("#valorCotacao___" + row)
            .val(addMascaraMonetaria(valor))
            .change();
          return false;
        }
      });
    ...
    configFornecedor: function (row) {
      ...
      $("#formaForn___" + row).append(
        '<option value="" disabled="disabled" selected="selected"></option> <option value="1">Boleto</option> <option value="2">Depósito Bancário</option> <option value="3">Ordem de Pagamento</option>'
      );

      if ($("#fornCadastrado___" + row).val() == 2) {
        $("#fornecedorProtheus___" + row).val("");
        $("#codForn___" + row).val("");
        $("#fornecedorProtheus___" + row)
          .parent()
          .parent()
          .hide();
        $("#fornecedorProtheusSemCadastro___" + row)
          .parent()
          .parent()
          .show();
      } else {
      ...
      $("#fornCadastrado___" + row).change(function () {
        ...
        if ($("#fornCadastrado___" + row).val() == "1") {
          ...
          $("#formaForn___" + row + " option[value='3']").prop(
            "disabled",
            false
          );
    ...
    comparacaoFornProd: function (row, listProduto, fornecedor) {
      var self = this;
      var compara = false;
      var rowForn = parseInt(
        $("[name^=cnpjForn___").last().attr("name").split("___")[1]
      );
      var rowProd = parseInt(
        $("[name^=codProd___").last().attr("name").split("___")[1]
      );
      if ($("[name^=codCotacao___").length > 0) {
        var rowCot = parseInt(
          $("[name^=codCotacao___").last().attr("name").split("___")[1]
        );
        if (rowForn * rowProd == rowCot) {
          for (i = 1; i <= rowCot; i++) {
            if (
              ($("#cnpjFornCotacao___" + i).val() ==
                $("#cnpjForn___" + row).val() ||
                $("#fornecedorCotacao___" + i).val() ==
                $("#nomeForn___" + row).val()) &&
              $("#codFornecedorCotacao___" + i).val() ==
              $("#codForn___" + row).val()
            ) {
              compara = true;
    ...
    gerarSolicitacaoDesvinculada: function () {
      ...
      var dsComprasDivers = getInDataset(
        "ds_comprasDiversos",
        ["codSolicitacao"],
        ["refSolicitacaoDesvinculada|" + self.form.codSolicitacao]
      );
      if (dsComprasDivers.values.length > 0) {
        self.form.codSolicitacaoDesvinculada =
          dsComprasDivers.values[0].codSolicitacao;
        FLUIGC.toast({
          message: "A solicitação com os itens desvinculados já existe. " +
            self.form.codSolicitacaoDesvinculada,
          type: "warning",
          timeout: "slow",
        });
      } else {
        var xmlSolicitacao = renderXmlSolicitacaoDiversos(
          self,
          self.getAllItem("Desvinculado"),
          "desvinculado"
        );
        self.iniciaNovaSolicitacao(
          xmlSolicitacao,
          ...
          function (data) {
            ...
            if (self.form.codSolicitacaoDesvinculada != "") {
              FLUIGC.toast({
                message: "A solicitação " +
                  self.form.codSolicitacaoDesvinculada +
                  " foi gerada a partir dos itens desvinculados.",
                type: "success",
                timeout: "slow",
              });
            } else {
              showMessage(
                "ERRO",
                "Erro para gerar a solicitação. Atualize a página ou contate o time de TI"
              );
              console.log(data);
            }
          },
    ...
    loadFilialFarm: function () {
      var dsGruposUsuario = getInDataset(
        "colleagueGroup",
        ["colleagueGroupPK.groupId"],
        ["colleagueGroupPK.colleagueId|" + this.form.idSolicitante],
        false
      );
      var grupoUsuario = "";
      for (var i in dsGruposUsuario.values) {
        ...
        if (item["colleagueGroupPK.groupId"].indexOf("FARM") != -1) {
          ...
          dsFilialGrupo = getInDataset(
            "ds_FilialGrupo",
            ["codFilial"],
            ["idGrupoFilial|Pool:Group:" + grupoUsuario]
          );
    ...
    setFilialByCode: function (codFilial) {
      var dsFilial = getInDataset("cad_Filiais", null, [
        "filial_protheus|" + codFilial,
      ]);
    ...
    checkVencedor: function (rowVencedor, linha) {

      console.log("check vencedor ");
      console.log("LINHA " + linha);
      console.log("ROW VENCEDOR " + rowVencedor);
      ...
      readColumn("codCotacao", function (row, item) {
        ...
        var checkLinha = $("#codCotacaoLinha___" + row).val();
        var checkCodVencedor = $("#codCotacao___" + row).val();

        if (linha == checkLinha && codVencedor == checkCodVencedor) {

          $("#vencedorCotacao___" + rowVencedor).prop("checked", true);
          $("#vencedorCotacao___" + rowVencedor).val("on");

          var codVencedorLinha = $("#codCotacaoLinha___" + rowVencedor).val();

          console.log("codVencedor " + codVencedor);
          console.log("codVencedorLinha " + codVencedorLinha);
        ...
        if (rowVencedor != row && codVencedor == item.val() && linha == checkLinha) {
    ...
    getItem: function (row, nomeTabela) {
      var produto = {
        ...
        codFornVencedorProd: $(
          "#codFornVencedorProd" + nomeTabela + "___" + row
        ).val(),
        fornecedorVencedor: $(
          "#fornecedorVencedor" + nomeTabela + "___" + row
        ).val(),
        isFechada: $("#isFechada" + nomeTabela + "___" + row).prop("checked") == true ?
          "on" : "",
        item: $("#item" + nomeTabela + "___" + row).val(),
      };
      return produto;
    },
    showModalCotacao: function (orderBy, value, titulo, subTitulo, linha) {
      readColumn(orderBy, function (row, item) {
        ...
        if (linha != $("#codCotacaoLinha___" + row).val() && orderBy == "codCotacao") {
    ...
    carregaValorProduto: function (filtro) {
      ...
      if (dsTabelaPreco.values[0].CODIGO != "") {
        ...
        if (itemAtual != null) {
          ...
          self.setValorCotacao(
            itemAtual.COD_FORNECEDOR,
            itemAtual.PRODUTO,
            itemAtual.VALOR
          );
        } else {
        ...
        FLUIGC.toast({
          message: "O produto " +
            codProduto[1] +
            " NÃO foi localizado nas tabelas de preços.",
          type: "warning",
          timeout: "slow",
        });
        console.log("VALOR DO PRODUTO NÃO ENCONTRADO");
    ...
    validarAlcadasCompras: function () {
      ...
      if (self.form.isFornecedorExclu == true && codFilial != "") {
        ...
        var constraintPai = DatasetFactory.createConstraint(
          "metadata#active",
          "true",
          "true",
          ConstraintType.MUST
        ...
        var datasetPai = DatasetFactory.getDataset(
          "ds_alcadaAprovComprasDiv",
          null,
          new Array(constraintPai),
          null
        ).values;
        for (var i in datasetPai) {
          ...
          // Busca o dataset
          var ds_aprovadores = DatasetFactory.getDataset(
            "ds_alcadaAprovComprasDiv",
            null,
        ...
        if (ds_aprovadores.length == 0) {
          ...
          self.form.isAlcadaMsg =
            self.form.isAlcadaMsg +
            "Não existe aprovadores para a carta de exceção desta compra.<br>";
        } else {
    ...
    setAlcadaAprovacaoDiversos: function () {
      ...
      if (correcaoSolicitacao == "false" && valor != "" && valor != "0") {
        ...
        for (var i in datasetPai) {
          ...
          // Busca o dataset
          var ds_centroCusto = DatasetFactory.getDataset(
            "ds_alcadas_aprov_v2",
            null,
            new Array(c1, c2, c3, c4),
            null
        ...
        if (valor > 50000) {
          ...
          // Busca o dataset
          var ds_cordCompras = DatasetFactory.getDataset(
            "ds_alcadas_aprov_v2",
            null,
            new Array(c1, c2, c3),
            null
        ...
        var ds_cad_Filiais = DatasetFactory.getDataset(
          "cad_Filiais",
          null,
          null,
          null
        ...
        if (
          self.form.isFornecedorExclu == true ||
          self.form.isFornecedorExcluClassica == true
        ) {
          for (var i = 1; i <= 5; i++) {
        ...
        if (ds_aprovadores.values.length > 0) {
          if (self.form.isFornecedorExcluClassica == false) {
            ...
            if (valor > 100000) {
              if (ds_cordCompras[0].usuarioGerenteFinanceiro == "") {
                self.form.isAlcadaDiversos = "erro";
                self.form.isAlcadaMsg =
                  "Não existe gerente financeiro cadastrado.";
              } else if (
                $("#idSolicitante").val() !=
                ds_cordCompras[0].usuarioGerenteFinanceiro
              ) {
                var aprovador = new Object();
            ...
            if (valor > 50000) {
              if (ds_cordCompras[0].usuarioCoordCompras == "") {
                self.form.isAlcadaDiversos = "erro";
                self.form.isAlcadaMsg =
                  "Não existe coordenador de compras cadastrado.";
              } else if (
                $("#idSolicitante").val() !=
                ds_cordCompras[0].usuarioCoordCompras
              ) {
                var aprovador = new Object();
          ...
          if (
            $("#idSolicitante").val() !=
            ds_aprovadores.values[0].usuarioGerente3
          ) {
            var aprovador = new Object();
          ...
          if (
            (primeiraAlcada ||
              ...
              removeMascaraMonetaria(
                ds_aprovadores.values[0].valorGerente3
              )) &&
            $("#idSolicitante").val() !=
            ds_aprovadores.values[0].usuarioOperacoes
          ) {
            var aprovador = new Object();
          ...
          if (
            (primeiraAlcada ||
              valor >
              removeMascaraMonetaria(
                ds_aprovadores.values[0].valorOperacoes
              )) &&
            $("#idSolicitante").val() !=
            ds_aprovadores.values[0].usuarioFinanceiro
          ) {
            var aprovador = new Object();
          ...
          if (
            (primeiraAlcada ||
              valor >
              removeMascaraMonetaria(
                ds_aprovadores.values[0].valorFinanceiro
              )) &&
            $("#idSolicitante").val() != ds_aprovadores.values[0].usuarioGeral
          ) {
            var aprovador = new Object();
          ...
          if (
            (primeiraAlcada ||
              ...
              1) &&
            $("#idSolicitante").val() != ds_aprovadores.values[0].usuarioGeral2
          ) {
            var aprovador = new Object();
        ...
        /* if ( problema aprovacao carta excecao
          self.form.isFornecedorExclu == true &&
          $("#alterouValorCarta").val() == "sim"
        ) {
          aprovCarta.forEach(function (item, index) {
    ...
    setAlcadaAprovacaoCarta: function () {
      ...
      if (
        (self.form.isFornecedorExclu == true && valor != "") ||
        (correcaoSolicitacao == "false" && valor != "")
      ) {
        var codFilial = $("#codFilial").val();
        ...
        if (ds_aprovadores.length > 0) {
          if (
            $("#idSolicitante").val() != ds_aprovadores[0].codAprovCoordSupri
          ) {
            var aprovador = new Object();
          ...
          if (self.form.isAlcadaCartaComprador != true) {
            if (
              valor > 100000 &&
              $("#idSolicitante").val() != ds_aprovadores[0].codAprovGestorFin
            ) {
              var aprovador = new Object();
          ...
          if (
            valor > removeMascaraMonetaria(ds_aprovadores[0].valorAlcada1) &&
            $("#idSolicitante").val() != ds_aprovadores[0].codAprov2
          ) {
            var aprovador = new Object();
          ...
          if (
            valor > removeMascaraMonetaria(ds_aprovadores[0].valorAlcada3) &&
            $("#idSolicitante").val() != ds_aprovadores[0].codAprov3
          ) {
            var aprovador = new Object();
    ...
    setAlcadaAprovacaoPA: function () {
      ...
      if (isPA == "true" && valor != "") {
        $("#alcada").val("PA");
        // Defina as constraints para buscar o ds do pai
        var constraints_principal = DatasetFactory.createConstraint(
          "metadata#active",
          "true",
          "true",
          ConstraintType.MUST
        );
        var dsAlcadaAprovPA = DatasetFactory.getDataset(
          "ds_alcadaAprovPA",
          null,
          new Array(constraints_principal),
          null
        ).values;
        ...
        for (var i in dsAlcadaAprovPA) {
          ...
          var c2 = DatasetFactory.createConstraint(
            "metadata#id",
            dsAlcadaAprovPA[i]["metadata#id"],
            dsAlcadaAprovPA[i]["metadata#id"],
            ConstraintType.MUST
          );
          var c3 = DatasetFactory.createConstraint(
            "metadata#version",
            dsAlcadaAprovPA[i]["metadata#version"],
            dsAlcadaAprovPA[i]["metadata#version"],
            ConstraintType.MUST
          );

          //Busca o dataset
          var ds_alcadaAprovPA = DatasetFactory.getDataset(
            "ds_alcadaAprovPA",
            null,
            new Array(c1, c2, c3),
            null
        ...
        for (j = 0; j < ds_alcadaAprovPA.length; j++) {
          if (
            valor >
            removeMascaraMonetaria(ds_alcadaAprovPA[j]["valorAlcada"]) &&
            $("#idSolicitante").val() != ds_alcadaAprovPA[j]["codAprov"]
          ) {
            var aprovador = new Object();
        ...
        if (
          jsonAprovadores.length < 4 &&
          $("#idSolicitante").val() == ds_alcadaAprovPA[0]["codAprov"]
        ) {
          var aprovador = new Object();
        ...
        if (gestores.length > 0) {
          ...
          self.form.isAlcadaMsg =
            "Não foi possivel carregar aprovadores para o PA, favor verificar cadastro de aprovadores!";
        }
      }
    },
    ...
    somaTotalProduto: function (rowProduto) {
      var self = this;
      var valorProduto = removeMascaraMonetaria(
        $("#valorProd___" + rowProduto).val()
      );
      ...
      FLUIGC.message.confirm({
        ...
        function (result, el, ev) {
          if (result == true) {
            ...
            $(
              "#workflowActions > button:first-child",
    ...
    calculaSaving: function () {
      ...
      if ($("#hideFornecedorExclu").val() != "true") {
        ...
        var valorTotal = removeMascaraMonetaria(
          self.form.valorTotalSolicitacao
        );
...
$(document).ready(function () {
  ...
  if (vueApp.atividadeAtual == 6) {
    $("#remetente").val(parent.WCMAPI.userCode);
  }

  if (
    vueApp.atividadeAtual == vueApp.process.inicio ||
    vueApp.atividadeAtual == 0
  ) {
    $("#msgEmerg").hide();
  ...
  if ($("#codFilial").val() == "00104") {
    //So liberar o contrato guardachuva para a filial HOLDING SP
    FLUIGC.switcher.enable("#isContratoGuardChuva");
    FLUIGC.switcher.enable("#isPedidoGuardChuva");

  } else {
    FLUIGC.switcher.setFalse("#isContratoGuardChuva");
    FLUIGC.switcher.disable("#isContratoGuardChuva");

    FLUIGC.switcher.setFalse("#isPedidoGuardChuva");
    FLUIGC.switcher.disable("#isPedidoGuardChuva");
  }
  ...
  $("#anexo5").change(function () {
    if (
      $("input[name='anexo5']:checked").val() == "on" ||
      $("input[name='anexo5']:checked").val() == "ON"
    ) {
      $("#anexoOutros").show();
    } else {
...
function regraContratoAnexo4() {
  if (
    $("input[name='anexo4']:checked").val() == "on" ||
    $("input[name='anexo4']:checked").val() == "ON"
  ) {
...
function setSelectedZoomItem(item) {
  ...
  // FILIAL PARA PREDIDO GUARDA-CHUVA
  else if (typer.indexOf("filialGuardChuPedido___") >= 0) {
    window["filialGuardChuPedido___" + index].value = item.CODIGO + " - " + item.DESCRICAO;
    console.log();
    $("#codFilialGuardChuPedido___" + index).val(item.CODIGO);
    $("#cnpjFilialPedido___" + index).val(item.CGC);

    // Bloqueia os campos enquanto a consulta é feita.
    if ($("#enderecoFilialPedido___" + index).val() == "") {
      $("#enderecoFilialPedido___" + index).addClass("loadinggif");
    }
    getDadosEnderecoPedido(index, item.CGC);
  }
  
  // FILIAL PARA CONTRATO GUARDA-CHUVA
  else if (typer.indexOf("filialGuardChu___") >= 0) {
    window["filialGuardChu___" + index].value = item.CODIGO + " - " + item.DESCRICAO;
  ...
  //CENTRO DE CUSTO
    ...
    //   showMessage(
    //     "Atenção!",
    //     "Filiais <b>'CSO e HOLDING'</b> não podem selecionar <b>'Centro de Custos'</b> iniciados por 2."
    //   );
    ...
    // if (codFilial == "00104" && contrato != "true" && codigoCC.indexOf("2") == 0) {
    //   showMessage(
    //     "Atenção!",
    //     "Filiais <b>'CSO e HOLDING'</b> não podem selecionar <b>'Centro de Custos'</b> iniciados por 2."
    //   );
    ...
    console.log($("codProd___" + index).val())

  }
  //PRODUTO
  else if (typer.indexOf("codProd") >= 0) { // == $("#codProd___" + index)[0].id) {

    /*reloadZoomFilterValues("codProd___" + index,'filial,07401');
    ...
    $("[name^=codProd___]").each(function () {
      ...
      if (item.CODIGO == $(this).val() && parseInt(item.CODIGO) <= 99000) {
        // produtoExistente = true;
      }
    });
    ...
    if (produtoExistente) {
      ...
      showMessage(
        "Atenção!",
        "Já foi encontrado o produto <b>" +
        item.CODIGO +
        " - " +
        item.DESCRICAO +
        "</b> na lista de produtos."
      );
      $("#codProd___" + index)
        .val("").on("change")
    } else {
      ...
      var pegaCod = item.CODIGO;
      //Verifica se o produto é matmed

      if (parseInt(item.CODIGO) >= 990000 /*&& $("#tipoCompra").val() != "3"*/ ) {
        FLUIGC.message.alert({
          message: 'Os itens de código igual ou superior a 990000 não podem ser selecionados.',
          title: 'Atenção !',
          label: 'Ok!'
        }, function (el, ev) {});
        window["codProd___" + index].setValue("null");

      } else if (parseInt(item.CODIGO) > 0 && parseInt(item.CODIGO) <= 880000 && ($("#codFilial").val() == '07401' || $("#codFilial").val() == '08401' || $("#codFilial").val() == '12201'
    	|| $("#codFilial").val() == '06601' || $("#codFilial").val() == '02901' || $("#codFilial").val() == '07301') &&
        $("#tipoCompra").val() == "2") {
        FLUIGC.message.alert({
          message: 'Para as unidades hospitalares, selecionar produtos com códigos maiores que 880000.',
          title: 'Atenção !',
          label: 'Ok!'
        }, function (el, ev) {});
        window["codProd___" + index].setValue("null");

      } else if (parseInt(item.CODIGO) > 880000 && ($("#codFilial").val() != '07401' && $("#codFilial").val() != '08401' && $("#codFilial").val() != '12201'
    	&& $("#codFilial").val() != '06601' && $("#codFilial").val() != '02901' && $("#codFilial").val() != '07301') && $("#tipoCompra").val() == "2") {
          FLUIGC.message.alert({
              message: 'Para as unidades não hospitalares, selecionar produtos com códigos menores que 880000.',
                  title: 'Atenção !',
                  label: 'Ok!'
              }, function (el, ev) {});
              window["codProd___" + index].setValue("null");              
      } else if (tipo == "MD" || tipo == "MM" || tipo == "MQ" || tipo == "SO") {
        showMessage(
          "Atenção!",
          "Produdos <b>MAT/MED</b> não podem ser selecionados neste fluxo. Favor verificar o produto ou abrir o fluxo de compras MAT/MED."
        );
        window["codProd___" + index].setValue("null");
        $("#nomeProd___" + index)
          .val("")
          .change();
        $("#unidadeProd___" + index)
          .val("")
          .change();
      } else if ($("#tipoCompra").val() == "1") {
        //Serviço
        if ($("#tbProd tbody tr").length == 2) {
          codServOuProd = "";
        }
        if (tipo == "SV" && pegaCod > "000100") {
          window["codProd___" + index].setValue(item.CODIGO);
          if ($("#nomeProd___" + index).val(item.DESCRICAO).change() && CURRENT_STATE == 6 && $("#newProd___" + index).val() == "") {
            // pegar as alterações feitas pelo comprador
            ...
            $("#nomeProd___" + index).val(item.DESCRICAO).change() && CURRENT_STATE == 6 && $("#newProd___" + index).val() == "sim") {
            $("#analiseProdutosAlterados").show("slow");
            var linha = wdkAddChild("tbProdAlterados");
          ...
          $("#unidadeProd___" + index)
            .val(item.UM)
            .change();
          $("#fabricanteProd___" + index)
            .val(item.FABRICANTE)
            .change();
          $("#contaContabilProd___" + index)
            .val(item.CCONTABIL)
            .change();
          $("#grupoProduto___" + index)
            .val(item.GRUPO)
            .change();
          ...
          showMessage("Atenção!", "este item não e permitido por se tratar de produtos ou <b>Serviços</b> de faturamento");
          $("#codProd___" + index)
            .val("")
            .change();
        }

      } else if ($("#tipoCompra").val() == "2") {
        //Produto
        if ($("#tbProd tbody tr").length == 2) {
          codServOuProd = "";
        }
        if (tipo != "SV") {
          ...
          if (tipo == "MD" || tipo == "MM" || tipo == "MQ" || tipo == "SO") {
            ...
            // nova validação para compras TI tmb
            /*
            if (parseInt(item.CODIGO) > 0 && parseInt(item.CODIGO) <= 880000 && ($("#codFilial").val() == '07401' || $("#codFilial").val() == '08401' || $("#codFilial").val() == '12201')) {
                FLUIGC.message.alert({
                      message: 'Para as unidades hospitalares, selecionar produtos com códigos maiores que 880000.',
                      title: 'Atenção !',
                      label: 'Ok!'
      ...
      } else if ($("#tipoCompra").val() == "3") {
        //TI
        if ($("#tbProd tbody tr").length == 2) {
          codServOuProd = "";
        }
        if (grupo == "0184" || grupo == "0159") {
          window["codProd___" + index].setValue(item.CODIGO);
          if (
            $("#nomeProd___" + index)
            .val(item.DESCRICAO)
          ...
          //showMessage(
          //"Atenção!",Solicitação só permite a inclusão de <b>Produtos e Serviços da TI</b>."
          //);
          //$("#codProd___" + index)
          //.val("")
          //.change();
        ...
        showMessage(
          "Atenção!",
          "Favor abrir solicitação pelo Sistemas de Chamados, selecionado as opções de <b>Produtos</b> ou <b>Serviços</b>"
        );
        $("#codProd___" + index)
          .val("")
          .change();
  ...
  //FORNECEDOR
  else if (typer.indexOf("fornecedorProtheus") >= 0) { // == $("#fornecedorProtheus___" + index)[0].id) {
    var aux = $("[id^='codForn___']")
      .toArray()
      .some((e, i, g) => {
    ...
    if (aux == true) {
      FLUIGC.toast({
        message: "Opção invalida. O c&oacute;digo do fornecedor " +
          item.CODIGO +
          " j&aacute; esta na lista de cota&ccedil;&atilde;o.",
        type: "danger",
        timeout: "slow",
      });
      ...
      $("#lojaForn___" + index)
        .val(item.LOJA)
        .change();
      $("#emailFornecedor___" + index)
        .val(item.EMAIL.trim())
        .change();
      $("#formaForn___" + index)
        .val(item.COD_FORM_PAGTO)
        .change();
      $("#bancoForn___" + index)
        .val(item.BANCO)
        .change();
      $("#agenciaForn___" + index)
        .val(item.AGENCIA)
        .change();
      $("#dvAgenciaForn___" + index)
        .val(item.DV_AGENCIA)
        .change();
      $("#contaForn___" + index)
        .val(item.CONTA)
        .change();
      $("#dvContaForn___" + index)
        .val(item.DV_CONTA)
        .change();
      $("#hiddenBancoForn___" + index)
        .val(item.BANCO)
        .change();
      $("#hiddenAgenciaForn___" + index)
        .val(item.AGENCIA)
        .change();
      $("#hiddenDvAgenciaForn___" + index)
        .val(item.DV_AGENCIA)
        .change();
      $("#hiddenContaForn___" + index)
        .val(item.CONTA)
        .change();
      $("#hiddenDvContaForn___" + index)
        .val(item.DV_CONTA)
        .change();
      $("#cnpjForn___" + index)
        .val(item.CGC)
        .change();
      $("#codIssFornec___" + index)
        .val(item.CODIGOS_ISS)
        .change();
    }
  }
  //FORNECEDOR EXCLUSIVO
  else if (typer == $("#fornecedorExclusProtheus")[0].id) {
  ...
  //AGLUTINAÇÃO
  else if ($("#isAglutinar")[0].checked) {
    if ($("table[tablename=tbAgl] tr").length - 2 > 0)
      if (typer == $("#codAgl___" + index)[0].id)
    ...
    $("#codAgl___" + index)
      .val(item.codSolicitacao)
      .change();
    $("#descAgl___" + index).val(item.campoIdentificador);
  }
  //CONDICAO PAGAMENTO
  else if (typer.indexOf("condForn") >= 0) { // == $("#condForn___" + index)[0].id) {
...
function getDadosEnderecoPedido(index, cnpj) {
  // Consulta o webservice receitaws.com.br/
  var cnpj = cnpj.replace(/\D/g, "");
  $.ajax({
    type: "get",
    ...
    if (data.status != "ERROR") {
      //Atualiza os campos com os valores da consulta.
      $("#enderecoFilialPedido___" + index).val(
        data.logradouro +
        " " +
        data.numero +
        " - " +
    ...
    $("#enderecoFilialPedido___" + index).removeClass("loadinggif");
  });
}
...
function validaGuardaChuva() {
  ...
  $("[name^=valorUnitario___]").each(function () {
    ...
    if (t > 100 && flg == 0) {
      showMessage(
        "Erro",
        "O valor do percentual total das filiais não podem ser maior que 100%."
      );
      $("#valorUnitario___" + indice).val("");
      flg = 1;
    }
...
function validaGuardaChuvaPedido() {
  var t = 0;
  var a = 0;
  var flg = 0;
  $("[name^=valorUnitarioPedido___]").each(function () {
    var indice = this.name.split("___")[1];
    a =
      $("#valorUnitarioPedido___" + indice).val() == "" ?
      "0" :
      $("#valorUnitarioPedido___" + indice).val();
    t = t + Number(a);
    if (t > 100 && flg == 0) {
  ...
  if (vueApp.edicao) {
    ...
    if ($("[name^=codProd___]").length > 1) {
      ...
      if (vueApp.form.tipoCotacao == "fechada") {
        deleteCotacao("codCotacao", $("codProd___" + row).val(), $("itemProd___" + row).val());
      }
      fnWdkRemoveChild(item);
    } else {
      FLUIGC.message.alert({
...
function deleteFornecedor(item) {
  ...
  readColumn("cnpjVencedorProd", function (rowProduto, item) {
    if (item.val() == cnpjForn) {
      ...
      $("#valorProd___" + rowProduto)
        .val(addMascaraMonetaria("0"))
        .change();
    }
  });
  //    cnpjFornCotacao___
  ...
  if (cnpjForn != "") {
    if (
      $("#hiddenQtdCotacoes").val() != "0" &&
      $("#fornecedorProtheus___" + row).val() != null
    ) {
      $("#hiddenQtdCotacoes").val(
        parseInt($("#tbFornecedor").find("tbody tr").length) - 1
      );
...
function deleteCotacao(nomeCampo, valor, linha) {
  var linhaAtiva = "";
  if (nomeCampo == "codCotacao") {
    readColumn(nomeCampo, function (row, item) {


      if (item.val() == valor && linha == $("codCotacaoLinha___" + row).val()) {
        fnWdkRemoveChild(item[0]);
      } else {
        linhaAtiva = row;
      }
...
// Realiza o Cálculo dos valores de cada filial
function calcValorFilialPedido(oElement) {
  fnWdkRemoveChild(oElement); // Função padrão da lixeira
  validaGuardaChuvaPedido();
  var valores = 0;
}

function tabelaFilialQuant(oElement) {
  fnWdkRemoveChild(oElement); // Função padrão da lixeira no GERAR TABELA/QUANTIDADE DE PRODUTOS
...
function verificaQtdMinimaCotacao() {
  ...
  if (
    ((somaTotal > 5000 && somaTotal <= 10000 && rowTbFornecedor < "2") || (somaTotal >= 10000 && rowTbFornecedor <= "2")) &&
    $("#hideFornecedorExclu").val() == "false"
  ) {
...
function verificaValorCartaExcec() {
  ...
  var valorTotalSolicitacao = removeMascaraMonetaria(
    $("#valorTotalSolicitacao").val()
  );

  if (
    valorTotalSolicitacao > valorCompraExclu &&
    $("#hideFornecedorExclu").val() == "true"
  ) {
...
function sendEmailCotacao() {
  ...
  if (tbPedidosGerados > 0) {
    $("#tbPedidosGerados tbody tr:gt(0)").each(function () {
      ...
      var nomeFornecedor = $.trim(
        $(this).find("[name^=nomefornecedorPedido]").val()
      );
      var emailFornecedor = $.trim(
        $(this).find("[name^=emailFornVencedorProd]").val()
      );
      ...
      if (emailFornecedor != "") {
        enviaCorpoEmail(
          codFilial,
          pedidoGerado,
          emailFornecedor + ";" + copiaEmailSolicitante,
          nomeFornecedor
        );
      } else {
        FLUIGC.message.alert({
    ...
    var codFilial = $("#codFilial").val();
    var pedidoGerado = $("#numPedidoCompras").val();
    var nomeFornecedor = $("#nomefornecedorPedido").val();
    var emailFornecedor = $("#emailFornVencedorProd").val(); 
    var copiaEmailSolicitante = $("#emailContatSolicitante").val();
    if (emailFornecedor != "") {
      enviaCorpoEmail(
        codFilial,
  ...
  function enviaCorpoEmail(codFilial, pedidoGerado, email, nomeFornecedor) {
    ...
    for (const key in anexos) {
      if (Object.hasOwnProperty.call(anexos, key)) {
        ...
        if (
          element.description.indexOf("Pedido_Compras_") > -1 &&
          element.description.indexOf(pedidoGerado) > -1
        ) {
    ...
    var cons = DatasetFactory.createConstraint(
      "JSON",
      JSON.stringify(json),
      "",
      ConstraintType.MUST
    );
    var ds_envioEmail = DatasetFactory.getDataset(
      "ds_envioEmail",
      null,
      [cons],
      null
...
function dadosVencedores() {
  ...
  $("#jsonFormatado").val(acompanhementos);
