	...
	if(CURRENT_STATE == INICIO_1 || CURRENT_STATE == INICIO || CURRENT_STATE == CORRIGIR_SOLICITACAO){
		...
		$(".zoomProduto").on("click", function(event) {
			openZoom("ds_produtos",
					...
					$(event.target).closest('td').last().find('input').attr('name').split('___')[1]);
		})
...
function setSelectedZoomItem(selectedItem) {
	var origem = selectedItem.inputId.split("___")[0];
	var index = parseInt(selectedItem.inputId.split("___")[1]);
	
	if (isNaN(index))
		index = 1;
	
	if(origem == 'zoomFilial'){
		var constraintDs_filial = DatasetFactory.createConstraint('CODIGO', selectedItem.CODIGO, selectedItem.CODIGO, ConstraintType.MUST);
		...
		$('#filial').val(selectedItem.CODIGO);
		$('#hiddenFilial').val(selectedItem.CODIGO);
        $("[name=F1_FILIAL]").val(result.CODIGO);
        $('#FilialAlcada').val(selectedItem.CODIGO);
        $("#nomeFilialEnt").val(selectedItem.CODIGO);
		...
		$('#txtCodCentroCusto').val(selectedItem.CODIGO);
		$('#txtNomeCentroCusto').val(selectedItem.DESCRICAO);
		
		getAprovadorCentroCusto(selectedItem.CODIGO);
		...
		$('#txtCodFornecCotacao___'+index).val(selectedItem.CODIGO).change();
		$('#txtNomeFornecCotacao___'+index).val(selectedItem.DESCRICAO).change();
		$('#txtLojaFornecCotacao___'+index).val(selectedItem.LOJA).change();
		$('#txtCnpjFornecCotacao___'+index).val(selectedItem.CGC).change();
	} else if(origem == 'solicitacoesCompras'){
		$('#txtNumSolicAdd___'+index).val(item.numSolicitacao).change();
	} else if(origem == 'condicaoPagamento' || origem == 'txtNomeFormaPagamentoBionexo'){
		$('#txtCodFormaPagamentoProtheus___'+index).val(selectedItem.CODIGO).change();
		$('#txtNomeFormaPagamentoBionexo___'+index).val(selectedItem.DESCRICAO).change();
