...
var configApp = {
   ...
   mounted: function() {
      ...
      if (this.atividadeAtual != this.process.inicio || this.atividadeAtual != this.process.analiseCompradorCotacao) {
      ...
      if (this.atividadeAtual == this.process.inicio || this.atividadeAtual == 0) {
         ...
         $("#nomeSolicitante").val(parent.WCMAPI.user);
         ...
         if (MODO_EDICAO != "VIEW") {
            ...
            readColumn("codProd", function(row) {
               ...
               $("#qtdProd___" + row).add("#valorProd___" + row).change(function() {
            ...
            if (self.form.tipoCotacao == "tabela") {
               ...
               if (self.listProdutoFechada.length > 0) {
                  var dsComprasDivers = getInDataset("ds_comprasDiversos",
                     ["codSolicitacao"],
        ...
        $("#defineSLA option")[3].selected = true;
      ...
      } else if (this.atividadeAtual == this.process.correcaoSolicitante || this.atividadeAtual == 5) {
         //CONFIGURACOES DOS BOTOES DA ALCADA DE APROVACAO
         flagAllBtnAprov();
         definePrioridade();
         var contadorAlcada = parseInt(self.form.contadorAlcada) - 1;
         ...
         addEventSendFluig(function() {
            ...
            if (self.form.alteraValorCartaExecao == "true" || self.form.isFornecedorExclu == true) {
      ...
      if (this.atividadeAtual != 0 && this.atividadeAtual != this.process.correcaoSolicitante) {
         self.form.correcaoSolicitacao = false;
      ...
      self.form.isPedidoGuardChuva = FLUIGC.switcher.getState("#isPedidoGuardChuva");
   ...
   methods: {
      ...
      addFilialPedido: function() {
         ...
         if (quantidadeProdutos < 2) {
            FLUIGC.toast({
               message: "Antes de adicionar as filiais para pedido de múltiplas partes, você deve adicionar pelo menos 1 produto. " + self.form.codSolicitacaoDesvinculada,
               type: "warning",
               timeout: "slow",
            });
         } else {
      ...
      addAglutinacao: function() {
         ...
         $("#codAgl___" + row).change(function() {
            ...
            if (algutinacaoExistente) {
               ...
               try {
                  var idTable = getInDataset("ds_comprasDiversos",
                     ["documentid"],
                     ["codSolicitacao|" + codSolicitacao]).values["0"].documentid;
                  var listProduto = getInDataset("ds_comprasDiversos", null, ["documentid|" + idTable, "tablename|tbProd", ]).values;
      ...
      openCotacao: function(item, tipoTabela) {
         if (MODO_EDICAO == "VIEW") {
            if (tipoTabela == "produto") {
               ...
               this.showModalCotacao("codCotacao", $("span#codProd___" + row).text(), "COTAÇÃO POR PRODUTO", $("span#nomeProd___" + row).text(), $("span#itemProd___" + row).text());
            ...
            if (tipoTabela == "fornecedor") {
               ...
               this.showModalCotacao("codCotacao", $("#codProd___" + row).val(), "COTAÇÃO POR PRODUTO", $("#nomeProd___" + row).val(), $("#itemProd___" + row).val());
            }
         }
      },
      ...
      comparacaoFornProd: function(row, listProduto, fornecedor) {
         ...
         if ($("[name^=codCotacao___").length > 0) {
            ...
            if (rowForn * rowProd == rowCot) {
               for (i = 1; i <= rowCot; i++) {
                  if (
                     ($("#cnpjFornCotacao___" + i).val() == $("#cnpjForn___" + row).val() || $("#fornecedorCotacao___" + i).val() == $("#nomeForn___" + row).val()) && $("#codFornecedorCotacao___" + i).val() == $("#codForn___" + row).val()) {
                     compara = true;
      ...
      loadFilialFarm: function() {
         var dsGruposUsuario = getInDataset("colleagueGroup",
            ["colleagueGroupPK.groupId"],
            ["colleagueGroupPK.colleagueId|" + this.form.idSolicitante], false);
         var grupoUsuario = "";
         for (var i in dsGruposUsuario.values) {
            ...
            if (item["colleagueGroupPK.groupId"].indexOf("FARM") != -1) {
               ...
               dsFilialGrupo = getInDataset("ds_FilialGrupo",
                  ["codFilial"],
      ...
      setAlcadaAprovacaoDiversos: function() {
         ...
         if (correcaoSolicitacao == "false" && valor != "" && valor != "0") {
            ...
            var ds_aprovadores = DatasetFactory.getDataset("ds_alcadas_diversos_2",
               [arrayFilial], null, null);
            ...
            if (ds_aprovadores.values.length > 0) {
               ...
               if (
                  (primeiraAlcada || valor > removeMascaraMonetaria(ds_aprovadores.values[0].valorGerente3)) && $("#idSolicitante").val() != ds_aprovadores.values[0].usuarioOperacoes) {
                  var aprovador = new Object();
               ...
               if (
                  (primeiraAlcada || valor > removeMascaraMonetaria(ds_aprovadores.values[0].valorOperacoes)) && $("#idSolicitante").val() != ds_aprovadores.values[0].usuarioFinanceiro) {
                  var aprovador = new Object();
               ...
               if (
                  (primeiraAlcada || valor > removeMascaraMonetaria(ds_aprovadores.values[0].valorFinanceiro)) && $("#idSolicitante").val() != ds_aprovadores.values[0].usuarioGeral) {
                  var aprovador = new Object();
      ...
      setAlcadaAprovacaoCarta: function() {
         ...
         var correcaoSolicitacao = $("#correcaoSolicitacao").val();
         if (
            (self.form.isFornecedorExclu == true && valor != "") || (correcaoSolicitacao == "false" && valor != "")) {
            var codFilial = $("#codFilial").val();
...
$(document).ready(function() {
   ...
   if ($("#orig_documentid"    ).val() == '' || $("#orig_documentid"    ).val() ==null || $("#orig_documentid"    ).val() =='0') $('#orig_documentid').val(getDocumentId());
   if ($("#orig_version"       ).val() == '' || $("#orig_version"       ).val() ==null || $("#orig_version"       ).val() =='0') $('#orig_version'   ).val(getVersion());
   if ($("#orig_docSolicitacao").val() == '' || $("#orig_docSolicitacao").val() ==null ) $('#orig_docSolicitacao').val('Documento');
   
   vueApp = new Vue(configApp);
   
   if($('#seAprovado').val() == "" || $('#seAprovado').val() == null)
	   $('#seAprovado').val("nao_confirmado");
   
   if($('#codFilial').val() != "")
		if($("#filial").val()=="" && $("#filial")[0].innerHTML==""){
			var constraintDs_filial = DatasetFactory.createConstraint('CODIGO',$('#codFilial').val(),$('#codFilial').val(),ConstraintType.MUST);
			var datasetDs_filial = DatasetFactory.getDataset('ds_filiais', null, [constraintDs_filial], null);
			
			$("#filial").val(datasetDs_filial.values[0].FILTRO);
		}  
   // LIMPAR PROGRAMACAO DE ONKEYPRESS DP FORMULARIO]
   $("#form").attr("onkeypress", "");
   // LIMPAR PROGRAMACAO DE ONKEYDOWN
   $("#form").attr("onkeydown", "");
   // LIMPAR PROGRAMACAO DE ONKEYUP
   $("#form").attr("onkeyup", "");
   
   if (CURRENT_STATE > INICIO) { 
		$('#classeOrcamentaria').prop('readonly', true); 
		$('#classeValor').prop('readonly', true); 
	} else { 
		$('#classeOrcamentaria').prop('readonly', false); 
		$('#classeValor').prop('readonly', false);
   }	
	
   // SE O MOD ESTIVER EM VIEW - ESCONDER BOTES (BTN)
   if(MODO_EDICAO == "VIEW")
      $('.btn').hide();
   ...
   if (vueApp.atividadeAtual == vueApp.process.inicio || vueApp.atividadeAtual == 0) {
      $("#msgEmerg").hide();
   ...
   // VALOR PADRAO CAMPOS PCO
	if( $('#auxTipoDespesa').val() == 'capex')
		$('#somenteCapEx').show();

   // PROJETO PCO - VERIFICAR ALTERACAO DO CAMPO tipoDespesa E GRAVAR O VALUE EM auxTipoDespesa 
   $('#tipoDespesa').on('change', function() {
      var tipoDespesaValue = $(this).val();
      $('#auxTipoDespesa').val(tipoDespesaValue);
      // MOSTRAR DIV somenteCapEx SE O VALOR DO CAMPO tipoDespesa FOR IGUAL A capex
      if (tipoDespesaValue === 'capex') {
         $('#somenteCapEx').show();
      } else {
         $('#somenteCapEx').hide();
      }
...
// POJETO PCO - VER SALDO DO PROHJETO CAPEX/FILIAL
function getSaldoProjetoCapex() {
   // CAMPOS OBRIGATORIOS codFilial, codClasseOrc, codClasseVlr, valorTotalSolicitacao - SE QUALQUER UM ESTIVER SEM VALOR MOSTRAR MSG DE ERRO E RETORNAR FALSE
   var VALOR_SOLICITACAO = removeMascaraMonetaria($("#valorTotalSolicitacao").val());
   VALOR_SOLICITACAO = parseFloat(VALOR_SOLICITACAO);
   if (isNaN(VALOR_SOLICITACAO)) VALOR_SOLICITACAO = 0;
   if ($("#codFilial").val() == "" || $("#codClasseOrc").val() == "" || $("#codClasseVlr").val() == "" || VALOR_SOLICITACAO <= 0) {
   ...
   // LER TODOS O ITENS DA TABELA tbProd E ENVIAR PARA CONSULTA DE SALDOS E ACUMULAR O VALOR TOTAL
   var total = 0;
   var rows = $("#tbProd tbody tr").length;
   for (var x1 = 1; x1 <= rows; x1++) {
      var FILIAL = $("#codFilial").val(),
         ITEM = $("#codProd___" + x1).val(),
         CLASSE_ORC = $("#codClasseOrc").val(),
         CLASSE_VLR = $("#codClasseVlr").val();
      var constraints = [DatasetFactory.createConstraint("FILIAL", FILIAL, '', ConstraintType.MUST),
         ...
         DatasetFactory.createConstraint("CLASSE_ORC", CLASSE_ORC, '', ConstraintType.MUST),
         DatasetFactory.createConstraint("CLASSE_VLR", CLASSE_VLR, '', ConstraintType.MUST)
      ];
      var dataset = DatasetFactory.getDataset("ds_saldoProjetoCapex", null, constraints, null);
      try {
         if (dataset.values.length > 0) {
            total += parseFloat(dataset.values[0].SALDO);
         }
      } catch (erro) {
         console.erro("Mensagem de erro:", erro);
      }
   }
   // SE NAO RETORNAR NADA OU VALOR FOR INSUFICIENTE, MOSTRAR MENSAGEM DE ERRO
   if (VALOR_SOLICITACAO > total) {
      FLUIGC.message.alert({
         message: "Saldo insuficiente para a solicitação. Favor verificar.",
         title: "Aviso",
         label: "OK"
      });
      return false;
...
function setSelectedZoomItem(item) {
   ...
   //PRODUTO
      ...
      if (produtoExistente) {
         ...
         $("#codProd___" + index).val("").on("change")
      } else {
         ...
         if (parseInt(item.CODIGO) >= 990000 /*&& $("#tipoCompra").val() != "3"*/ ) {
         ...
         } else if (parseInt(item.CODIGO) > 0 && parseInt(item.CODIGO) <= 880000 && ($("#codFilial").val() == '07401' || $("#codFilial").val() == '08401' || $("#codFilial").val() == '12201' || $("#codFilial").val() == '06601' || $("#codFilial").val() == '02901' || $("#codFilial").val() == '07301') && $("#tipoCompra").val() == "2") {
            FLUIGC.message.alert({
            ...
            window["codProd___" + index].setValue("null");
         } else if (parseInt(item.CODIGO) > 880000 && ($("#codFilial").val() != '07401' && $("#codFilial").val() != '08401' && $("#codFilial").val() != '12201' && $("#codFilial").val() != '06601' && $("#codFilial").val() != '02901' && $("#codFilial").val() != '07301') && $("#tipoCompra").val() == "2") {
            FLUIGC.message.alert({
...
function getDadosEnderecoPedido(index, cnpj) {
   ...
   $.ajax({
      ...
      if (data.status != "ERROR") {
         //Atualiza os campos com os valores da consulta.
         $("#enderecoFilialPedido___" + index).val(data.logradouro + " " + data.numero + " - " + data.bairro + " - " + data.municipio + "/" + data.uf + " - " + data.cep);
      } else {
         showMessage("Erro de CNPJ - ", data.message);
...
function validaGuardaChuvaPedido() {
   ...
   $("[name^=valorUnitarioPedido___]").each(function() {
      var indice = this.name.split("___")[1];
      a = $("#valorUnitarioPedido___" + indice).val() == "" ? "0" : $("#valorUnitarioPedido___" + indice).val();
      t = t + Number(a);
      if (t > 100 && flg == 0) {
         showMessage("Erro", "O valor do percentual total das filiais não podem ser maior que 100%.");
...
function verificaQtdMinimaCotacao() {
   ...
   if (
      ((somaTotal > 5000 && somaTotal <= 10000 && rowTbFornecedor < "2") || (somaTotal >= 10000 && rowTbFornecedor <= "2")) && $("#hideFornecedorExclu").val() == "false") {
...
function sendEmailCotacao() {
   ...
   function enviaCorpoEmail(codFilial, pedidoGerado, email, nomeFornecedor) {
      ...
      var ds_envioEmail = DatasetFactory.getDataset("ds_envioEmail", null,
         [cons], null).values;
